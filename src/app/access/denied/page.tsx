...
export default async function DeniedAccessPage(): Promise<JSX.Element> {
  const session = await auth();
  const site = getPublicSiteConfig();

  if (!session?.user?.id) {
    redirect("/sign-in?next=/access/denied");
  }
      ...
      <main className="mx-auto w-full max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        <Card className="space-y-4">
          ...
          <h1 className="text-3xl font-semibold text-text-strong">
            Your current access has been restricted.
          </h1>
          <p className="text-sm text-text-muted">
            You are signed in, but this account does not currently meet the
            requirements for active agent access.
          </p>
          <p className="text-sm text-text-muted">
